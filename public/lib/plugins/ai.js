...
Kiwi.Plugins.AI.simpleAiAct = function(actor, target) {
	var directions = [ { x: -1, y:0 }, { x:1, y:0 }, { x:0, y: -1 }, { x:0, y:1 } ];	
	var dx = target.tileX - actor.tileX;
	var dy = target.tileY - actor.tileY;
...
Kiwi.Plugins.AI.smartAiAct = function(actor, target) {
	var start = this.walkabilityMap.nodes[actor.tileX][actor.tileY];
	var end = this.walkabilityMap.nodes[target.tileX][target.tileY];
	...
	var dir = { x: result[0].x-actor.tileX, y: result[0].y-actor.tileY};
